...
const getMostLikedImageUrl = (galleryKey) => {
    ...
    const mostLikedImage = sorted[0];
    const dir = galleries[galleryKey].dir;
    // FIXED: Use thumbnail URL instead of full image URL
    return createThumbnailUrl(dir, mostLikedImage.imageData);
};

// NEW: Refresh gallery covers function
const refreshGalleryCovers = () => {
    console.log('🔄 Refreshing gallery covers...');
    
    // Disconnect cover observer to prevent race conditions
    if (coverObserver) {
        coverObserver.disconnect();
        coverObserver = null;
    }
    
    // Clear any pending lazy load attributes
    document.querySelectorAll('.gallery-cover[data-bg]').forEach(cover => {
    ...
    Object.keys(galleries).forEach(key => {
        ...
        if (cover && galleryImageData[key]) {
            ...
            if (mostLikedUrl) {
                console.log(`🎨 Setting ${key} cover to: ${mostLikedUrl}`);
    ...
    console.log('✅ Gallery covers refreshed');
};
...
const setupGallerySelector = async () => {
    ...
    // NEW: Use refreshGalleryCovers to set initial covers
...
const closeGallery = () => {
    ...
    setTimeout(() => {
        // NEW: Refresh covers before showing gallery selector
        refreshGalleryCovers();
...
const setupCanvasNavigation = () => {
    ...
    document.addEventListener('keydown', function(e) {
        ...
        // FIXED: Don't handle Escape if modal is open
        ...
        switch(e.key) {
            ...
            case 'ArrowDown':
                ...
                break;
            case 'Escape':
                // FIXED: Check if modal is open, if so, close it instead
